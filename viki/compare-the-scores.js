...
function compareScores(candidateScores) {
  //Confirm if there is a non empty list of candidate .
  //if not bail out and return an empty array.
  ...
  // Give every person a point  staring at 0.
  ...
  const result = pointsMap.map((entry) => ({
    name: entry.name,
    points: entry.points,
    percentage: Number(((entry.points / numCategories) * 100).toFixed(2)),
  }));

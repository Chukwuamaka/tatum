...
const validateInput = () => {
  ...
  const emailIsValid = isInputValid(email, "email");
  const passwordIsValid = isInputValid(password, "password");
  ...
  if (emailIsValid && passwordIsValid)
    return { email, password, proceedToLogin: true };
};
...
function handleLogin(e) {
  e.preventDefault();
  const { proceedToLogin, ...requestData } = validateInput();
  if (proceedToLogin) login(requestData);
}

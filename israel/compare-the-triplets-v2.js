...
function compareScores(candidateScores) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    for (let i = 0; i < individualPoint.length; i++) {
      if (individualPoint[i].name === currentMaxScore.name &&
        equalMaxValue === false
      ) {
        individualPoint[i].points++;
        individualPoint[i].percentage = (individualPoint[i].points / 3 * 100).toFixed(2)
      }
    }
...
console.log(
  compareScores([
    ...
    { name: "Israel", scores: [8, 5, 3] },
    { name: "Michael", scores: [7, 6, 3] },
  ]),
);
